    ...
    public void DelObj(string type)
    {
        ...
        for (int index = 0; index < targetPool.Length; index++)
        {
            if(targetPool[index].activeSelf && type == "spaceship"
                && (Camera.main.WorldToViewportPoint(targetPool[index].transform.position).x <= -0.1f ||
                Camera.main.WorldToViewportPoint(targetPool[index].transform.position).x >= 1.1f))
